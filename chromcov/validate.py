"""
Input preflight: fail fast, with actionable messages, before any coverage runs.
...
On (2): the finite-difference *arithmetic* is order-independent (+1/-1 then
cumsum -- commutative), so it is NOT the coverage math that needs sorting. It's
the indexed region fetch: `.crai` only indexes a coordinate-sorted CRAM and
`fetch(chrom)` only works on one. So the true chain is fetch -> index -> sorted.

On (4): CRAM stores read bases as *differences* against the reference, so the
wrong reference can silently reconstruct wrong bases (not always an error). Each
@SQ line carries an M5 (MD5 of the uppercased sequence). We compare those to the
reference -- preferring a `.dict` sidecar (no re-hashing) over reading the .fa.

Both backends benefit: mosdepth needs sorted+indexed+correct-reference too.
"""
